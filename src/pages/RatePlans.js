...
const EMPTY_FORM = {
  hotelId: '',
  rateplanid: '',
  name: '', // Maps to Description.Name
  description: '', // Maps to Description.Text
  mealPlanId: '15', // Default: Room Only
  closeOutDays: '-1', // Default: No closeouts
  closeOutTime: '-1', // Default: No closeouts
};

const CLOSE_OUT_DAYS_OPTIONS = Array.from({length: 31}, (_, i) => i.toString()); // 0 to 30 strings
...
const RatePlanFormModal = ({ open, mode, hotelId, hotels, initialData, onClose, onSaved }) => {
  ...
  const validate = () => {
    ...
    // Logic: If CloseOutDays is 0, CloseOutTime is required
  ...
  const handleSubmit = async (e) => {
    ...
    const notifType = isCreate ? 'New' : 'Overlay'; // API Doc: To create use New, to Modify use Overlay
        ...
        RatePlan: [
          {
            ...
            // Only send closeout info if meaningful
...
const RatePlansPage = () => {
  ...
  const [statusDialog, setStatusDialog] = useState({ open: false, item: null, newStatus: '' });
  ...
  const loadRates = useCallback(async () => {
    ...
    try {
      ...
      // ⭐ FIX: Updated to match your API response structure
      const rawList = res?.data?.rateplans || []; // Correct path based on your JSON
      
      const mappedList = rawList.map((rp) => ({
        ...rp,
        rateplanid: rp.rateplanid,
        name: rp.rateplanname,     // Maps API 'rateplanname' to UI 'name'
        description: rp.description || '',
        mealPlanId: rp.mealplanid, // Note lowercase 'mealplanid' from API
                 ...
                 <tr key={rate.rateplanid}>
                   ...
                   <td className="rp-col-action-cell">
                      <ActionMenu 
                        ...
                        onActivate={() => setStatusDialog({ open: true, item: rate, newStatus: 'Active' })}
                        onDeactivate={() => setStatusDialog({ open: true, item: rate, newStatus: 'Deactivated' })}
